...
- **`pnpm shoot` works — two setup traps make it look broken.** (Playwright chromium launches fine; the previous "SIGTRAP, never claim visual verification" note here was stale and contradicted the mandatory visual inspection below.)
  ...
  - **Dark mode is resolved TWO ways, and picking the wrong one for the file renders light.** The global decorator in `.storybook/preview.tsx` reads `context.globals.theme`, but ~40 story files carry their own decorator reading `parameters.theme` instead (no file uses `globals` today). Setting `parameters.theme` in a file WITHOUT that local decorator renders light, and the capture looks fine — so "checked in dark" gets claimed on a light screenshot. Check how the file you are editing resolves it before trusting a dark shot. Same failure class as the wrong story id below: the shot succeeds, it is just not of the thing you meant.
  - **A wrong story id screenshots Storybook's "story not found" page and the script reports OK.** Ids are kebab-case. Verify every capture actually shows the component before citing it as evidence.
  - **A story with no explicit viewport parameter never exercises the mobile branch.** `.storybook/test-runner.ts` runs every story at 1280×720, so a responsive fix needs a story with the viewport set (and ideally a `play` asserting the mobile branch rendered) or the regression net does not exist.
