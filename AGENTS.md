...
- **`pnpm shoot` works — two setup traps make it look broken.** (Playwright chromium launches fine; the previous "SIGTRAP, never claim visual verification" note here was stale and contradicted the mandatory visual inspection below.)
  ...
  - **Dark mode needs `globals: { theme: 'dark' }`, not `parameters.theme`.** A story setting `parameters.theme` renders in LIGHT and the capture looks fine, so "checked in dark" gets claimed on a light screenshot. Same failure class as the wrong story id below: the shot succeeds, it is just not of the thing you meant.
