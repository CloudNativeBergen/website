...
`ticketTargets.salesStartDate` becomes a read of `ticketSalesStartDate` (migration: copy existing
values, then remove the field from the targets object).
...
| Dashboard widget           | `defineWidget` in `src/lib/dashboard/widget-registry.ts` (`MARKETING_DUE_WIDGET`, category `operations`), a case in `src/components/admin/dashboard/widget-renderer.tsx`, component under `src/components/admin/dashboard/widgets/`, data via `src/lib/dashboard/fetchers.ts`                                                                                                                                                   |
