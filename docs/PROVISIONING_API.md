...
Platform-operator standing is an authorization input, and it must not hinge on anything a tenant can edit. It used to key on `PLATFORM_ORG_SLUG`: the slug is a customer-writable field (edited through kontroll), resolved to an org id by an uncached Sanity lookup on `slug.current`. Production has one organization that is both the platform org and a tenant, so an admin editing its slug in kontroll could move operator standing out from under the platform — and while the slug lived, the gate paid a Sanity read on every evaluation.

Since RunKonf/platform#43 the contract is `PLATFORM_ORG_ID` — the platform org's **immutable document id**. `getPlatformOrgId()` (`src/lib/authz/platform.ts`) returns that configured id directly: pure env, **no Sanity read**, and every caller compares ids against it. There is no slug lookup to go stale and no mutable field a tenant could edit to gain or lose standing. This cache-invalidation endpoint exists for content, not for grants.
