...
| Ticket types   | Offered only the types named "Sponsor…", so a free code is not pointed at full-price public inventory — but an event with no such type is offered all of them, and a type already ticked stays listed | Any type, or all of them                        |
