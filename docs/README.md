...
- **[Organization Tier (Multi-Tenant Foundation)](./ORGANIZATION_TIER.md)** - The `organization` tenant model: global-person speaker membership, denormalized tenant keys for reference-blind document security, creation-path stamping, the additive backfill, and what waves 2/3 build on it (CaaS T1-1)
- **[Tenant-Scoped Query Invariant](./TENANT_SCOPING.md)** - The rule that every tenant read is scoped to one organization/conference: the `scopedFetch` builder + `CONFERENCE_FILTER`/`ORG_FILTER` predicates, the `tenancy/no-unscoped-groq` lint rule and its two-marker annotation vocabulary (`// groq-global:` for reviewed cross-tenant reads, `// groq-global-scoped:` for reads the rule cannot see the scope of), org-aware speaker identity resolution, and the incremental migration playbook (CaaS #615, #616)
- **[Integration Adapters](./INTEGRATION_ADAPTERS.md)** - The house pattern for third-party integrations behind a provider interface: the `TicketingProvider` and `ContractSigningProvider` contracts, the credential-injection rule (providers never read `process.env`), the request-boundary resolver, and the Checkin-shaped types a second ticketing provider must generalize
- **[The Control Model](./CONTROL_MODEL.md)** - How this app, `RunKonf/kontroll` (my.konf.app) and one shared Sanity dataset relate: the write partition and kontroll's `PortalWriter` choke point, the two bearer-authenticated machine endpoints and why their rate limits are asymmetric, cache coherence across two writers, tenant-scoping enforcement and its known limits, the append-only schema lock, and the failure modes — including the ones not yet designed for
- **[Machine Provisioning API](./PROVISIONING_API.md)** - `POST /api/provisioning/organizations`, the bearer-authenticated tenant-creation endpoint RunKonf/kontroll calls: the request/response contract, the fail-closed shared-secret auth model, the two durable rate limits, and the transaction-level idempotency receipt
- **[Customer Onboarding Checklist](./ONBOARDING_CHECKLIST.md)** - The operator runbook from invite to live conference: the platform prerequisites (`PROVISIONING_API_TOKEN`, `PLATFORM_DOMAIN_SUFFIX`, the mail identity), the kontroll invite → redemption → `/setup` path, what the provisioning transaction does automatically, the manual Vercel domain attach that has no code, per-step verification and remedies, and the steps whose outcome is not observable at all (RunKonf/platform#42)
- **[Per-Organization Secrets](./TENANT_SECRETS.md)** - The per-org secret resolution layer + storage interface: the typed credential families, the `TenantSecretsStore` interface with its env-default and `TENANT_SECRETS_JSON` per-org stores, the per-org→env→null fallback chain, the wired consumers (ticketing/email/slack), and what #617's later management-UI wave adds
- **[Web Analytics (PostHog)](./ANALYTICS.md)** - Per-organization PostHog cutover from Pirsch: the token gate, the proxied client init and hybrid consent bar, the CTA event catalogue and the PostHog Actions to create, and what /privacy declares
- **[Marketing Plan — slice 1 specification](./MARKETING_PLAN_SPEC.md)** - The implementation-ready spec for the Marketing Plan admin feature (PostHog measurement edition): Campaigns and Tasks seeded from the Plan Template, Bluesky (integrated) and LinkedIn (manual) publishing, the non-publishing Task Kinds, Outcomes and daily Snapshots, the per-edition Marketing Report, and the dashboard widget. Vocabulary lives in the marketing section of the repository-root [`CONTEXT.md`](../CONTEXT.md)
