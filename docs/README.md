...
- **[Tenant-Scoped Query Invariant](./TENANT_SCOPING.md)** - The rule that every tenant read is scoped to one organization/conference: the `scopedFetch` builder + `CONFERENCE_FILTER`/`ORG_FILTER` predicates, the `tenancy/no-unscoped-groq` lint rule and its `// groq-global:` suppression convention, org-aware speaker identity resolution, and the incremental migration playbook (CaaS #615, #616)
