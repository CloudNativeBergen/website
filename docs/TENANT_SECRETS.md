...
| buffer    | `resolveSocialCredentials(orgId, 'linkedin')` (`src/lib/social/provider/index.ts`)  | **Wired (#1129).** The resolved bag IS the organization's LinkedIn connection: the publish cron builds a `BufferPublishAdapter` on the pinned channel from it; a missing or half-filled bag means manual LinkedIn variants. A Buffer account belongs to one tenant organization, so this family is per-org only (`TENANT_<SLUG>_BUFFER_*`) with no platform-env fallback.                                                                                                |
