...
- **Migration.** Existing plans are backfilled: for each Campaign whose `key` matches a built-in
  Campaign, copy that Campaign's `2026.1` Recipes onto it. Dry-run then apply, as migration 052.
  `generatedKeys[]` already records what has been created, so a backfilled Recipe cannot create a
  Task twice; that property is the one the tests must prove by sabotage.
...
shown live; typing a plain date suggests the nearest Milestone and the offset. An anchored manual Task
is re-dated with the edition like any seeded Task (slice 1 §2.6). Tasks created before this, and Tasks
...
organization-owned, read with `ORG_FILTER`. Fields: `organization`, `templateId` (stable across
versions), `name` (unique per organization), `version` (integer, 1…n), `campaigns[]` with their
Recipes and static Tasks, and provenance `savedFrom` (conference), `savedBy`, `savedAt`,
...
### 4.2 Built-in Campaigns as presets
...
signed sponsor, talk teaser per scheduled talk, video drip per recorded talk, countdown. The Library
...
**In:** Campaigns (title, window anchors, Outcome, target page, optional flag, stored Recipes); static
Tasks of origin `template`, `copy` and `manual` (title, Kind, Channel, anchor, Prerequisites,
instructions, target page, alt). **Out:** assignees, status, approvals, assets, published posts,
...
4. **Recipe Library and built-in Campaign presets** — §4, §5. Needs 2.
