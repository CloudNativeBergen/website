...
| **Organizer inbox**        | `/admin/messages` → `MessagesInbox audience="organizer" allowNew`                                                                            | Scrollable tablist: **`active`** (default), **`needs-reply`**, **`my-teams`** ("My teams"), **`unassigned`**, **`mine`**, **`resolved`**, **`archived`**. `all` is a server capability, deliberately **not** surfaced as a tab. The **My teams** tab (TEAMS-3) appears only when a team lens is configured and shows threads routed to a team the viewer belongs to. Rows show counterpart, unread pill, "Needs reply" dot, `Resolved` chip, an assignee badge, and (TEAMS-3) an indigo routing-team chip. |
| **Inbox rows**             | `ConversationList`                                                                                                                           | One `<Link>` per row to `conversationLinkPath(item, isOrganizer)`; in the Archived view rows gain an inline **Unarchive** (organizer lifts both global + per-user; speaker lifts per-user). Organizer rows carry a **team chip** (TEAMS-3, `deriveThreadTeamChip`): the sponsor thread's chip shows the `sponsors`-team title (falling back to "Sponsor"), and a `cfp`-routed thread shows the named `cfp`-team chip — orthogonal to the gray type chip.                                                   |
