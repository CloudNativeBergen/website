...
Safari and Firefox on a computer. Everything below was **measured** on 2026-09-23; nothing is
promised beyond what was run. The throwaway page, the run logs and the analyses lived in commit
[`7d65c68a`](https://github.com/CloudNativeBergen/website/commit/7d65c68af7279893122f8ad3a4145e1e48ce48f9)
(`scratch/video-proof/`) and were removed before merge.
...
returns nothing: no output chunk, no error, and `flush()` never resolves. It happens with raw
WebCodecs and no Mediabunny at all, and with `prefer-software`, level 4.0 and Baseline 3.1 too. It
...
- **Speed:** encoding the 10 s stereo track took 650–820 ms in every browser (Chrome 746–823,
  Safari 646–661, Firefox 727–787). It runs in a worker, and the page's longest task stayed at
  56 ms or less. At that rate a 60 s track is about 4–5 s.
...
| Longest main-thread task during export                 | 20–32 ms (165 ms once, first run after launch)                                              | 16–35 ms                                        | 29–56 ms (85 ms silent)                         |
| Frames painted during export (`requestAnimationFrame`) | 64–266                                                                                      | 141–180                                         | 86–187                                          |
| Cancel                                                 | real click at 20 %: handler ran, `output.cancel()` resolved in **1.6 ms**, state `canceled` | timer task: delivered in 1 ms, resolved in 3 ms | timer task: delivered in 4 ms, resolved in 0 ms |

The progress bar painted all the way through, and input got through during the export in every
browser. One Safari run happened with the window not visible: 0 frames painted and timers
throttled. It still finished, in 6.5 s instead of 2.4, which matches the spec's reason for avoiding
...
audio packet at 0 and **no edit list**. So every file starts with the encoder's priming silence,
and the sound comes late by that much. Clicks were measured in ffmpeg's decode against the
timestamp of the matching flash frame, on every click in the file:
...
Every click in a file had the same offset. Two ways to fix it were tried:

- **Measure at export time, then trim (recommended).** Encode half a second containing one click
  with the same encoder, decode it back through Mediabunny, and find the click. That took 24–163 ms
...
- **Safari's encoder reports `fullRange: true`**, but the pixels it writes are limited range.
  ...
  off (`#1d4ed8` becomes about `#2952cd`). How LinkedIn's and Bluesky's transcoders read it is
  **not known**.
...
| Bitrate, whole file | **234–455 kbit/s** (Chrome native 253, Safari 234, Firefox 455) | **192 kbit/s – 30 Mbit/s**                                                       | not stated                                                                                                               |

**Watch the bitrate.** The target is 8 Mbit/s, but on a flat design the encoders spend far less:
Chrome's video track alone is 177 kbit/s. That is already close to LinkedIn's 192 kbit/s minimum,
and a stiller design could fall under it. #1177 should check the real bitrate of a typical studio
design, and set a minimum (a floor on the quality, or constant bitrate) if needed.
...
(`scratch/video-proof/out/chrome-auto-1.mp4`, `safari-rt-auto-1.mp4`, `firefox-auto-1.mp4`) as a
...
It worked in all three browsers, costs about 254 KB gzip fetched only when needed, and adds about
0.7 s per 10 s of sound, off the main thread. If the licence is not accepted, **export silent with
a clear notice** ("this browser cannot add the music; use Chrome, Edge or Safari for sound").
Telling people to switch browser should be the last resort, since the add-on is proven to work.
...
1. **Latency mode per browser.** Safari needs `'realtime'`, Firefox must not get it, and Chrome is
   ...
   size in `'quality'`: encode a handful of frames and require at least one output chunk within
   about a second. If none arrives, retry in `'realtime'`. If that also fails, refuse.
...
Also for #1177: check the bitrate against LinkedIn's 192 kbit/s minimum (§7), and decide whether to
correct Safari's `fullRange: true` label (§5). It may be enough to raise it with Mediabunny or
WebKit.
...
is 1024. Measure it at export time (§4: 24–163 ms) and trim that many samples from the start of the
mixed track. #1179 should also carry the add-on decision above, including the LGPL notice on
`/privacy` or the licences page if the add-on ships.
