...
function isSpeaker(speaker: Speaker | unknown): speaker is Speaker {
  return speaker !== null && typeof speaker === 'object' && speaker !== undefined && 'name' in speaker
}
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4">
        <h2 className="text-lg font-semibold text-gray-900">Proposal Preview</h2>
        <button
          onClick={onClose}
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 gap-4 text-sm">
            <div className="flex items-center space-x-2">
              ...
              <span className="font-medium">{formatDuration(proposal.format)}</span>
            </div>
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="font-medium">{formatLanguage(proposal.language)}</span>
            </div>

            {proposal.audiences && proposal.audiences.length > 0 && (
              <div className="flex items-start space-x-2">
                <span className="text-gray-600">Audience:</span>
                <span className="font-medium">{formatAudience(proposal.audiences)}</span>
              </div>
            )}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {proposal.topics.map((topic) => {
                  ...
                  const topicTitle = typeof topic === 'object' && 'title' in topic
                    ? topic.title
                    : typeof topic === 'object' && '_ref' in topic
                      ? 'Topic'
                      : 'Unknown Topic'
                  const topicId = typeof topic === 'object' && '_id' in topic
                    ? topic._id
                    : typeof topic === 'object' && '_ref' in topic
                      ? topic._ref
