...
export function ImageGrid({
  ...
  const handleToggleFeatured = async (imageId: string, targetFeatured: boolean) => {
  ...
  return (
    <>
      <div className="mb-4 flex items-center justify-between">
        ...
        {selectedImages.length > 0 && (
          <div className="flex items-center gap-2">
            <button
              onClick={async () => {
                const toToggle = images.filter(img => selectedImages.includes(img._id))
            ...
            >
              {images.filter(img => selectedImages.includes(img._id)).every(img => img.featured) 
                ? 'Unfeature Selected' 
                : 'Feature Selected'}
            </button>
            <button
          ...
          >
            <div className="absolute left-2 top-2 z-10">
              <button
                ...
                className="rounded bg-white bg-opacity-80 p-1 shadow-sm hover:bg-opacity-100"
              >
                <div className={`h-4 w-4 rounded border ${
            ...
            <div className={`absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 to-transparent p-3 transition-opacity ${
              hoveredImage === image._id ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'
            }`}>
            ...
            <div className={`absolute inset-x-0 top-1/2 flex -translate-y-1/2 justify-center gap-2 transition-opacity ${
      ...
      <ConfirmationModal
        ...
        title={deleteConfirmId === 'bulk' ? `Delete ${selectedImages.length} Images` : "Delete Image"}
