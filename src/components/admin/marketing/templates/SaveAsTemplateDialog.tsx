...
export function SaveAsTemplateDialog({ onClose }: { onClose: () => void }) {
  ...
  // What was SUBMITTED, not what the form says when the answer arrives.
  ...
  const save = api.marketing.template.save.useMutation({
    onSuccess: (result) => {
      ...
      showNotification({
        type: 'success',
        title: `Saved “${submittedName.current}” as version ${result.version}`,
      })
      onClose()
    },
  })
  ...
  const submit = () => {
    if (blocked) return
    // The save answers THIS list: the server refuses it if the plan has since
    // gained an item that was never shown here.
    const answered = {
      fingerprint: preview.data!.fingerprint,
      decisions: saveDecisions(review, drafts),
    }
    if (mode === 'version' && templateId) {
      submittedName.current = chosen?.name ?? ''
      save.mutate({ ...answered, target: { type: 'version', templateId } })
    } else if (mode === 'new') {
      submittedName.current = name.trim()
      save.mutate({ ...answered, target: { type: 'new', name: name.trim() } })
    }
  }
  // Rewritten copy, moved anchors and a typed name are work: Escape, the
  // backdrop and the X ask before throwing them away.
  const dirty =
    name.trim() !== '' ||
    Object.keys(drafts.anchors).length > 0 ||
    Object.keys(drafts.copy).length > 0
  ...
  return (
    ...
    >
      ...
      >
        {/* Frozen while the save runs: what is on screen is what was sent. */}
        <fieldset
        ...
        >
          ...
          {(preview.data?.unsavedTargets.length ?? 0) > 0 && (
            <div className="rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-sm text-amber-900 dark:border-amber-700 dark:bg-amber-950/40 dark:text-amber-100">
              ...
              <p className="mt-1">
                A Template keeps a Target as a share of ticket capacity, and
                this edition has no ticket capacity set. Set it in the
                conference settings first, or save without{' '}
                {preview
          ...
          {preview.data && review.length === 0 && (
            <p className="rounded-md border border-dashed border-gray-300 px-3 py-6 text-center text-sm text-gray-600 dark:border-gray-700 dark:text-gray-300">
              Nothing needs a decision. Every Task is anchored to a Milestone
              and keeps the copy skeleton it was created from.
            </p>
          )}
          ...
          {copy.length > 0 && (
            <section className="space-y-3">
              ...
              <p className={NOTE}>
                ...
                2026&rdquo; becomes <code>{'{date}'}</code> — and it is filled
                in for the next one. Text left as it is here is saved word for
                word, and the Task it seeds is flagged for review.
              </p>
              <ul className="divide-y divide-gray-100 dark:divide-gray-800">
                {copy.map((item) => (
