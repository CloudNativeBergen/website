...
function CountdownInline({ remaining }: { remaining: number | null }) {
  ...
  return (
    <p
      className="font-space-grotesk flex flex-wrap items-baseline justify-center gap-x-3 text-2xl font-bold text-brand-slate-gray tabular-nums sm:text-3xl dark:text-gray-100"
      role="timer"
      aria-live="off"
    >
      {units.map((u) => (
        <span key={u.label} className="whitespace-nowrap">
          {u.value}
          <span className="font-inter ml-1 text-sm font-medium text-brand-slate-gray/60 dark:text-gray-400">
            {u.label}
          </span>
        </span>
      ))}
