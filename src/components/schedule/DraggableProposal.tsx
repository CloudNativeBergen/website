...
export function DraggableProposal({
  ...
  const { dragType, durationMinutes, talkSize, dragId, speakerInfo } = useMemo(() => {
    const duration = getProposalDurationMinutes(proposal)
    const type = sourceTrackIndex !== undefined ? 'scheduled-talk' : 'proposal'
  ...
  const containerClasses = useMemo(() => {
    const baseClasses = 'relative max-w-full overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm transition-shadow duration-200 hover:shadow-md'
    const opacityClass = isBeingDragged ? 'opacity-30' : isDragging ? 'opacity-50' : ''
    const borderClass = sourceTrackIndex !== undefined ? 'border-l-4 border-l-blue-500' : ''
    const paddingClass = talkSize === 'short' || talkSize === 'very-short' ? 'p-1' : 'p-2'
  ...
  const TitleComponent = useMemo(() => {
    ...
    switch (talkSize) {
      case 'very-short':
        return (
          <h3 className={`line-clamp-1 text-xs leading-tight font-medium ${titleClasses}`}>
            {proposal.title}
          </h3>
        )
      case 'short':
        return (
          <h3 className={`line-clamp-2 text-xs leading-tight font-medium ${titleClasses}`}>
            {proposal.title}
          </h3>
        )
      default:
  ...
  return (
    ...
    >
      ...
      <div className="flex min-h-[16px] items-start gap-1">
        {/* Drag handle */}
        <div
          className="flex-shrink-0 cursor-grab rounded p-0.5 hover:cursor-grabbing hover:bg-gray-100 transition-colors"
          {...listeners}
        >
        ...
        <div className="flex flex-shrink-0 items-center gap-1 text-xs text-gray-500">
          <ClockIcon className={talkSize === 'short' || talkSize === 'very-short' ? 'h-2.5 w-2.5' : 'h-3 w-3'} />
