...
export function MessageSlideOver() {
  ...
  // Determine if this is a proposal conversation by fetching some basic info,
  // or we could let the ConversationThread fetch it and expose it via context?
  // Since we don't have the context here, we can fetch the conversation type via TRPC.
  // Actually, we can just fetch the conversation header.
...
function ProposalPreviewWrapper({ proposalId }: { proposalId: string }) {
  ...
  return (
    <div className="space-y-4">
      <ProposalPreview proposal={data} onClose={() => {}} />
    </div>
  )
}
