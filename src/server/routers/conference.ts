...
export const conferenceRouter = router({
  ...
  updateHomepageSections: adminProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      // Client-supplied keys are kept only while UNIQUE — a duplicate would
      // corrupt Sanity array addressing and React reconciliation, so dupes are
      // dropped here and regenerated by ensureArrayKeys below.
      const seenKeys = new Set<string>()
      const sections = ensureArrayKeys(
        input.homepageSections.map((section) => {
          ...
          if (
            '_key' in section &&
            section._key &&
            !seenKeys.has(section._key)
          ) {
            seenKeys.add(section._key)
            base._key = section._key
          }
          ...
          switch (section._type) {
            ...
            case 'homepageRichText': {
              if (section.heading) base.heading = section.heading
              base.content = ensureArrayKeys(section.content, 'block')
              break
            }
            default:
