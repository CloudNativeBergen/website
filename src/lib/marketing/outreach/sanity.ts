...
import { taskDocument } from '../sanity'
import type { SeedTask } from '../seed'
...
export async function createOutreachTask(task: SeedTask) {
  await clientWrite.create(
    taskDocument(task, { _type: 'reference', _ref: task.conferenceId }),
  )
}
