...
function buildChecks(conference: ConferenceForSystemChecks): SystemCheck[] {
  ...
  checks.push({
    ...
    detail:
      fixedOrigins.length > 0
        ? 'next-auth rewrites EVERY request origin to this host — sign-in breaks on every other conference domain. Unset it; use AUTH_REDIRECT_PROXY_URL for a central OAuth origin.'
