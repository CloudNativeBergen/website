...
export async function handleGalleryTagNotification(
  ...
  const logLevel = failureCount > 0 && successCount === 0 ? 'error' : failureCount > 0 ? 'warn' : 'info'
  const logData = {
    imageId: event.image._id,
  ...
  if (logLevel === 'error') {
    logger.error('Gallery tag notification handling failed for all speakers', logData)
    ...
    logger.warn('Gallery tag notification handling completed with some failures', logData)
  } else {
