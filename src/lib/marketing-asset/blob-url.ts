...
/** The folder every marketing-asset blob of any organization sits in. */
export const MARKETING_ASSET_BLOB_PREFIX = 'marketing-asset/'

/**
 * A Sanity document id: `[A-Za-z0-9._-]`. Never `/`, which is what makes the
 * `/` after the organization id an unambiguous end to it: no organization id
 * can extend another's past that slash.
 */
const PLAIN_ID = /^[A-Za-z0-9._-]+$/

/** One organization's folder of pending uploads. */
export function marketingAssetPrefix(orgId: string): string {
  return `${MARKETING_ASSET_BLOB_PREFIX}${orgId}/`
}

/** Inside the folder: a 13-digit ms timestamp, then a plain filename. */
const AFTER_PREFIX = /^\d{13}-[A-Za-z0-9-]+(\.[A-Za-z0-9]+)?$/

/**
 * Whether `pathname` (no leading slash) names a blob in THIS organization's
 * folder, in the shape {@link marketingAssetPathname} builds. The token route
...
export function isMarketingAssetPathname(
  ...
  if (!PLAIN_ID.test(orgId)) return false
...
/**
 * Our Blob store's public host. The store id comes from `BLOB_READ_WRITE_TOKEN`
 * (`vercel_blob_rw_<storeId>_<secret>`) first, because that is the store
 * `handleUpload` signs every client token for, so it is where every browser
 * upload lands. `BLOB_STORE_ID` is only a fallback for a deployment without the
 * token. `null` when neither yields a plain id.
 */
export function blobStoreHost(
  ...
  const storeId = fromToken || fromId
