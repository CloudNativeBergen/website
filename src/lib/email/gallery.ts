...
export async function sendGalleryTagEmail(
  ...
  try {
    ...
    if (!imageUrl && image.image?.asset) {
      ...
      logger.warn('Image URL missing and cannot be built from asset reference', {
        imageId: image._id,
    ...
    if (!conference.title || !conference.city || !conference.start_date || !conference.cfp_email) {
      return {
        success: false,
    ...
    const socialLinks = conference.social_links && conference.social_links.length > 0 
