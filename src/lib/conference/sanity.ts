...
import { getFeaturedGalleryImages, getGalleryImages } from '@/lib/gallery/sanity'
...
export async function getConferenceForDomain(
  ...
  try {
    ...
    if (conferenceData) {
      ...
      if (gallery) {
        const galleryOptions = typeof gallery === 'object' 
          ? gallery 
          : { featuredLimit: 8, limit: 50 }
        
        const [featuredGalleryImages, galleryImages] = await Promise.all([
          getFeaturedGalleryImages(galleryOptions.featuredLimit ?? 8, revalidate, conference._id),
          getGalleryImages({ limit: galleryOptions.limit ?? 50, conferenceId: conference._id }, revalidate),
        ])
      ...
      if (gallery) {
        ...
        const [featuredGalleryImages, galleryImages] = await Promise.all([
          getFeaturedGalleryImages(galleryOptions.featuredLimit ?? 8, revalidate),
