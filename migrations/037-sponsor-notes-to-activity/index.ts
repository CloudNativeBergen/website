import { at, defineMigration, unset, createIfNotExists, patch } from 'sanity/migrate'

export default defineMigration({
  title: 'Migrate Sponsor Notes to Activity',
  description: 'Converts legacy notes field on sponsorForConference documents to sponsorActivity documents of type note',
    ...
    for await (const doc of documents()) {
      if (doc.notes && typeof doc.notes === 'string' && doc.notes.trim() !== '') {
        ...
        yield createIfNotExists({
          ...
          createdAt: doc._updatedAt || doc._createdAt || new Date().toISOString(),
        })
