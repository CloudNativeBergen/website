...
function categorizeFile(
  ...
  if (
    normalizedPath.includes('sanity/schematypes') ||
    normalizedPath.includes('sanity/schemas')
  ) {
...
function analyzeRepository() {
  ...
  const grandTotal = sortedCategories.reduce(
    (sum, [, stats]) => sum + stats.totalLines,
    0,
  )
  ...
  for (const [category, categoryStats] of sortedCategories) {
    const percentage = ((categoryStats.totalLines / grandTotal) * 100).toFixed(
      1,
    )
    console.log(
    ...
    if (Object.keys(categoryStats.subcategories).length > 0) {
      ...
      for (const [subcategory, subStats] of sortedSubcategories) {
        const subPercentage = (
          (subStats.lines / categoryStats.totalLines) *
          100
        ).toFixed(1)
        console.log(
  ...
  for (const [category, files] of stats.entries()) {
    ...
    if (topFiles.length > 0 && topFiles[0].lines > 100) {
      ...
      for (const file of topFiles) {
        console.log(
          `  ${file.lines.toLocaleString().padStart(6)} lines - ${file.path}`,
        )
      }
      console.log()
    }
